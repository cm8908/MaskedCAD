...
def get_dataloader(phase, config, shuffle=None):
    ...
    dataset = DropCLCadMLMDataset(phase, config)
    dataloader = DataLoader(dataset, batch_size=config.batch_size, shuffle=is_shuffle, num_workers=config.num_workers,
                            worker_init_fn=np.random.seed())
    return dataloader

class DropCLCadMLMDataset(CADDataset):
    def __init__(self, phase, config):
        super(DropCLCadMLMDataset, self).__init__(phase, config)
    ...
    def __getitem__(self, index):
        ...
        if self.phase in ['train', 'validation']:
            ...
            command_org = torch.LongTensor(cad_vec[:, 0])
            args_org = torch.LongTensor(cad_vec[:, 1:])
            tgt_cmd = torch.LongTensor(cad_vec[:, 0])
            tgt_args = torch.LongTensor(cad_vec[:, 1:])
            return {
                'command_masked': tokens[:, 0],
                'args_masked': tokens[:, 1:],
                'command': command_org,
                'args': args_org,
                'mask_labels': labels,
                'tgt_cmd': tgt_cmd,
